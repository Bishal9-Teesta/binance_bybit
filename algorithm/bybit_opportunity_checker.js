...
const file_path = path.join(__dirname, "../log/Bybit_Opportunity.csv")

const bybit_opportunity_checker = (binance_ask_pair_array, bybit_bid_pair_array, future_price, expiry_in_days, binance_symbol, bybit_symbol) => {
    ...
    if (!binance_ask_pair_array || !bybit_bid_pair_array) return
    ...
    while (should_continue) {
        ...
        if (
            (binance_pointer >= binance_ask_pair_array.length || !binance_ask_pair_array[binance_pointer]) ||
            (bybit_pointer >= bybit_bid_pair_array.length || !bybit_bid_pair_array[bybit_pointer])
        ) break

        let binance_quantity = binance_remaining_quantity === 0 ? Number(binance_ask_pair_array[binance_pointer][1]) : binance_remaining_quantity
        let bybit_quantity = bybit_remaining_quantity === 0 ? Number(bybit_bid_pair_array[bybit_pointer][1]) : bybit_remaining_quantity

        let minimum_quantity = Math.min(binance_quantity, bybit_quantity)

        const binance_ask_price = Number(binance_ask_pair_array[binance_pointer][0])
        const bybit_bid_price = Number(bybit_bid_pair_array[bybit_pointer][0]) + global.TWEAKER
        const spread = bybit_bid_price - binance_ask_price
        ...
        if (spread > global.SPREAD_MARGIN) {
            ...
            const expense = binance_fee + bybit_fee + binance_settlement_fee + bybit_settlement_fee
            const option_buy_premium = binance_ask_price * minimum_quantity
            const option_sell_premium = (future_price * minimum_quantity) * global.BYBIT.option_sell_margin
            ...
            if (opportunity > 0) {
                ...
                Binance Ask Price:                              ${binance_ask_price}
                Bybit Bid Price:                                ${bybit_bid_price}
    ...
    if (final.length > 0) {
        ...
        let data_to_log = `
            ...
            Binance Ask:                                    ${JSON.stringify(binance_ask_pair_array)}
            Bybit Bid:                                      ${JSON.stringify(bybit_bid_pair_array)}
            Spreads:                                        ${spreads}
        ...
        // Logger
        let ask = '['
        binance_ask_pair_array.forEach(pair => {
            ask = ask + `[${pair[0]}-${pair[1]}]`
        })
        ask = ask + ']'
        let bid = '['
        bybit_bid_pair_array.forEach(pair => {
            bid = bid + `[${pair[0]}-${pair[1]}]`
        })
        bid = bid + ']'
        // Binance Contract, Bybit Contract, Expiry, Future Price, Spreads, Accumulated_Quantity, Accumulated_Opportunity, Accumulated_Buy_Premium, Accumulated_Sell_Premium, Accumulated_Required_Margin, Trade_ROI, Trade_CAGR, Binance_Ask_Orderbook, Bybit_Bid_Orderbook
...
export default bybit_opportunity_checker
